...
export default function SeccionPagos({ estilosTema, perfil }: SeccionPagosProps) {
  ...
  const handleSubmitPago = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        registrado_por: perfil?.correo || 'Supervisor',
        en_papelera: false
      };
  ...
  const totalPaginasPagos = Math.ceil(pagos.length / elementosPorPagina) || 1;
  const indexUltimoPago = paginaActualPagos * elementosPorPagina;
  const pagosPaginados = pagos.slice(indexUltimoPago - elementosPorPagina, indexUltimoPago);
    ...
    <div className={`${estilosTema.bgCard} p-6 rounded-2xl border shadow-sm space-y-5`}>
            ...
            {pagosPaginados.map((p) => (
              <tr key={p.id} className="hover:bg-stone-500/10 transition-colors">
                ...
                <td className="py-4 px-4 text-right space-x-2">
                  {p.comprobante_url && (
                    <a href={p.comprobante_url} target="_blank" rel="noopener noreferrer" className="bg-stone-300/60 hover:bg-stone-300 px-3 py-2 rounded-xl font-bold inline-block">
                      🧾 Ver Voucher
                    </a>
                  )}
                  <button onClick={() => { setModalAtenderPago(p); setNuevoEstadoPago(p.estado_pago); setObsPagoInput(p.observacion_pago || ''); }} className={`px-3 py-2 rounded-xl font-bold shadow-md ${estilosTema.accentPrimary}`}>
                    ✏️ Gestionar
                  </button>
                </td>
              </tr>
      ...
      {totalPaginasPagos > 1 && (
        <div className="flex justify-between items-center pt-4 border-t border-stone-300/40 text-xs">
          <span className={estilosTema.subtext}>Mostrando página {paginaActualPagos} de {totalPaginasPagos} ({pagos.length} registros)</span>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className={`${estilosTema.bgCard} rounded-3xl max-w-md w-full p-8 space-y-5 text-xs border shadow-2xl`}>
            ...
            <form onSubmit={handleSubmitPago} className="space-y-4">
              ...
              <div>
                <label className="block font-bold mb-1 uppercase text-[11px]">🧾 Adjuntar Voucher (Opcional):</label>
